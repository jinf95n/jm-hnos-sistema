    ...
    <div className="p-4 sm:p-8 bg-slate-50 min-h-screen space-y-8 text-slate-900">
      ...
      {providers.length === 0 && (
        <div className="bg-amber-50 border border-amber-200 p-6 rounded-[2rem] flex items-center gap-4">
          ...
          <div>
            <p className="font-black text-amber-800 text-sm uppercase">
              Base de datos sin inicializar
            </p>
            <p className="text-amber-600 text-xs">
              Andá a la sección de "Importar" para cargar tus primeras listas de
              precios.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <section className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 h-fit">
          ...
          <div className="space-y-4">
            {providers.length > 0 ? (
              ...
              <p className="text-slate-300 text-xs font-bold text-center py-4">
          ...
          <Link
            href="/admin/import"
            className="mt-8 block text-center py-5 border-2 border-dashed border-slate-200 rounded-[1.5rem] text-slate-400 font-black text-[10px] uppercase tracking-widest hover:border-blue-500 hover:text-blue-600"
          >
            Importar Listas
          </Link>
        </section>
        ...
        {/* Consultas */}
        <section className="bg-white p-6 sm:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 lg:col-span-1">
          <div className="flex items-center gap-3 mb-8">
          ...
          <div className="space-y-6">
            {pendingOrders.length > 0 ? (
              pendingOrders.map((order) => (
                <div
                  key={order.id}
                  className="p-5 sm:p-6 bg-slate-50 rounded-[2rem] border border-slate-100"
                >
                  <div className="flex justify-between items-start mb-6 gap-2">
                    <div className="min-w-0">
                      {" "}
                      {/* min-w-0 evita que el nombre rompa el flex */}
                    ...
                    <span className="text-green-600 font-black text-sm shrink-0 bg-green-100/50 px-2 py-1 rounded-lg">
                      $
                      ...
                      <div className="space-y-4">
                        ...
                        <ul className="space-y-3 mb-6">
                          {order.items.map((item: any) => (
                            <li
                              key={item.id}
                              className="flex flex-col gap-1 border-b border-slate-200/50 pb-2 last:border-0 last:pb-0"
                            >
                              <div className="flex gap-2 items-start">
                                <span className="font-black text-[#103f79] text-[11px] shrink-0 bg-blue-100/50 px-1.5 rounded">
                                  {item.quantity}
                                </span>
                                <span className="text-[11px] font-bold uppercase leading-tight text-slate-700 whitespace-normal break-words">
                                  {item.product?.name}
                                </span>
                              </div>
                            </li>
              ...
              <p className="text-slate-300 text-xs font-bold text-center py-10 uppercase tracking-widest">
